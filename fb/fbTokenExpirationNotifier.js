...
exports.notify = async (event) => {
    await slackClient.chat.postEphemeral({ thread_broadcast: false, thread_ts: event.thread_ts, channel: event.channel, user: event.user, text: 'Your message was not posted on FB due to fb token expiration. A notification is also sent to your system administrator to refresh the token. Meanwhile sit back and try later!' })
